...
library LinkedLists {
    ...
    struct Lists {
        // nonce => keccak256(nonce, role) 
        mapping(uint256 => bytes32) heads;
        // keccak256(nonce, role) => ListItem
        mapping(bytes32 => ListItem) items;
    }
    ...
    function insert(Lists storage _self, uint256 _headKey, bytes32 _nonce, IERCXXXX.RoleData memory _data) internal {
        require(_nonce != EMPTY, "LinkedLists: invalid nonce");

        bytes32 headNonce = _self.heads[_headKey];
        if (headNonce == EMPTY) {
            // if list is empty
            // insert as head
    ...
    function _insertAt(Lists storage _self, bytes32 _previousNonce, bytes32 _dataNonce, IERCXXXX.RoleData memory _data) internal {
    ...
    function remove(Lists storage _self, uint256 _headKey, bytes32 _nonce) internal {
        bytes32 headNonce = _self.heads[_headKey];
        ...
        if (_self.items[_nonce].previous == EMPTY) {
            ...
            if (_self.items[_nonce].next == EMPTY) {
                ...
                // set new head
                bytes32 newHeadNonce = _self.items[_nonce].next;
